...
function ready(){
    ...
    var addToCartButtons = document.getElementsByClassName('ksrbtn');
...
function purchaseClicked(){
    alert('Köszönjük hogy nálunk vásároltál!');
...
function removeCartItem(event) {
    var buttoClicked = event.target;
    buttoClicked.parentElement.parentElement.remove();
    updateCartTotal();
}
...
function addToCartClicked(event) {
    ...
    var imageSrc = shopItem.getElementsByClassName('smallpic')[0].src;
...
function addItemToCart(title, price, imageSrc){
    ...
    for (var i = 0; i < cartItemNames.length; i++) {
        if (cartItemNames[i].innerHTML == title) {
            alert('Ez a termék már hozzá lett adva a kosárhoz!')
            return
        }
    }
    var cartRowContents = `
